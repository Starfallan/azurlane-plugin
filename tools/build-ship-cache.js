...
import { buildShipCache } from '../model/ship-cache-builder.js'
...
async function main() {
  ...
  const result = await buildShipCache({ force, only, limit, delayMs, retries, retryDelayMs })
  console.info(`[build:ships] 已写入 ${result.count} 条舰船缓存，失败 ${result.failed ?? 0} 条。`)
}

main().catch((error) => {
  console.error('[build:ships] 构建失败:', error)
  process.exitCode = 1
})
