...
STATE_FILE = ROOT / "resources" / "character" / "build-state.json"
NODE_SCRIPT = ROOT / "tools" / "build-ship-cache.js"
...
def archive_and_reset_state_file():
    if STATE_FILE.exists():
        ...
        print(f"[build-all-ship] 已归档旧状态文件到: {backup}")
...
def get_pending_items():
    ...
    for entry in entries:
        ...
        if original_name in completed:
            continue
        if original_name in failed_names:
...
def run_batch(batch, delay_ms: int, retries: int, retry_delay_ms: int, force: bool):
    ...
    try:
        ...
        print(f"[build-all-ship] 启动批次: {len(batch)} 条")
...
def main():
    parser = argparse.ArgumentParser(description="批量构建舰船缓存，分批调用 Node 构建器并利用 build-state.json 续跑。")
    ...
    if args.force:
        ...
        print("[build-all-ship] 已创建全新舰船抓取状态，将从头覆盖旧数据。")
    ...
    while True:
        ...
        if not pending:
            print("[build-all-ship] 没有待处理舰船，任务完成。")
            return 0

        batch = pending[:args.batch_size]
        batch_count += 1
        print(
            f"[build-all-ship] 第 {batch_count} 批，待处理总数 {len(pending)}，"
        ...
        if code != 0:
            print(f"[build-all-ship] 批次执行失败，退出码 {code}")
            return code

        if args.max_batches > 0 and batch_count >= args.max_batches:
            print("[build-all-ship] 已达到 max-batches，停止。")
            return 0
