...
async function loadLocalRequestConfig() {
  try {
    const raw = await fs.readFile(EQUIP_REQUEST_CONFIG_FILE, 'utf8')
...
export async function buildEquipRequestHeaders(url) {
  const localConfig = await loadLocalRequestConfig()
  const envCookie = process.env.BLHX_EQUIP_COOKIE ?? ''
...
export async function fetchWithRetry(url, options = {}) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      const headers = await buildEquipRequestHeaders(url)
